    ...
    try {
        ...
        if (req.user.cargo_id == 4) {
            ...
            const [assinaturas] = await db.execute(
                ...
                JOIN equipa_documentos ed ON ed.equipa_id = em.equipa_id
                JOIN equipas e ON e.id_equipa = em.equipa_id
                LEFT JOIN equipa_assinaturas ea 
                    ON ea.conta_id = em.conta_id 
                    AND ea.documento_id = ed.documento_id
                WHERE ed.documento_id = ?
                AND e.tipo = 'revisores'`,
                [idDocumento]
            );

            todosAssinaram = assinaturas.every(a => a.assinou_documento == 1);
        ...
        res.json({ success: true, todosAssinaram });
    } catch (err) {
        console.error(err);
