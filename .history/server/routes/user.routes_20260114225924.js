    ...
    try {
      ...
      if (req.user.cargo_id !== 4) {
        ...
        const [[doc]] = await db.execute(
            `SELECT erros_encontrados FROM documentos WHERE id_documento = ?`,
            [idDocumento]);

        
        const [assinaturas] = await db.execute(
            `SELECT COALESCE(ea.assinou_documento, 0) AS assinou
        ...
        const todosAssinaram = assinaturas.every(a => a.assinou === 1);

        
        if (!todosAssinaram) {
            await db.execute(
            `UPDATE documentos
            SET estado = 'aguardando_assinaturas'
            WHERE id_documento = ?`,
            [idDocumento]);

            return res.json({ success: true, todosAssinaram: false });
        }

        
        if (doc.erros_encontrados === 'Sem Erros') {
            await db.execute(
            `UPDATE documentos
            SET estado = 'finalizado',
                documento_link_final = documento_link_traduzido
            WHERE id_documento = ?`,
            [idDocumento]);
        } else {
            await db.execute(`UPDATE documentos
            SET estado = 'em_traducao'
            WHERE id_documento = ?`,
            [idDocumento]);

            // aqui deve tirar a associaçao da equipa revisores a documento
            await db.execute(`
                DELETE`, [idDocumento]);

        }
        res.json({ success: true, todosAssinaram: true });
      }
    } catch (err) {
      console.error(err);
...
router.put('/documentos/:idDocumento/revisao', verifyToken, verifyRole([4]), async (req, res) => {
    const { idDocumento } = req.params;
    const { erros_encontrados } = req.body; 
