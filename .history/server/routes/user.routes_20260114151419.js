...
router.put('/documentos/:idDocumento/assinatura', verifyToken, verifyRole([3, 4]), async (req, res) => {
    ...
    if (!idDocumento || !conta_id || typeof assinou_documento === 'undefined') {
     ...
     try {
        const [result] = await db.execute(
            `UPDATE equipa_assuinaturas
             SET assinou_documento = ?
             WHERE documento_id = ? AND conta_id = ?`,
            [assinou_documento, idDocumento, conta_id]
        );
