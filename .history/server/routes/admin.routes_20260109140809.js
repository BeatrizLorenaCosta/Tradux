...
router.post('/documentos/associar', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const [[doc]] = await db.query(
            `SELECT id_documento, esta FROM documentos WHERE id_documento = ?`,
            [documento_id]
        );
        ...
        if (equ.tipo === 'tradutores') {
            await db.query(
                `UPDATE documentos SET estado = 'em_traducao' WHERE id_documento = ? `,
                [documento_id]
            );
        } else if (equ.tipo === 'revisores') {
            await db.query(
                `UPDATE documentos SET estado = 'em_revisao' WHERE id_documento = ? `,
                [documento_id]
            );
        }
