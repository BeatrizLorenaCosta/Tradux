...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      // 1️⃣ Registar assinatura (tradutor ou revisor)
      await db.execute(
      ...
      // 2️⃣ Só revisores decidem estados
      if (req.user.cargo_id !== 4) {
        return res.json({ success: true });
      }

      // 3️⃣ Buscar erros do documento
      const [[doc]] = await db.execute(
        `SELECT erros_encontrados FROM documentos WHERE id_documento = ?`,
        [idDocumento]
      );

      // 4️⃣ Verificar se TODOS os revisores assinaram
      ...
      // 6️⃣ Todos assinaram → decidir pelo conteúdo da revisão
