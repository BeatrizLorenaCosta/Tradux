...
router.put('/documentos/:idDocumento/revisao', verifyToken, verifyRole([4]), async (req, res) => {
    ...
    const idConta = req.user.id;

    try {
        // 1️⃣ Verificar se o documento existe e quem é o responsável
        ...
        // 2️⃣ Apenas o responsável pode guardar revisão
        if (rows[0].conta_id !== idConta) {
            return res.status(403).json({ message: 'Sem permissão para rever este documento' });
        }

        // 3️⃣ Atualizar erros_encontrados
        await db.execute(
            `UPDATE documentos
