    ...
    try {
        const [rows] = await db.query(`
            SELECT 
                ...
                COALESCE(ea.assinou_documento, 0) AS assinou_documento,
            ...
            LEFT JOIN equipa_assinaturas ea ON ea.conta_id = c.id_conta AND ea.documento_id = d.id_documento
        ...
        rows.forEach(r => {
            ...
            if (r.id_documento && !documentosMap.has(r.id_documento)) {
                const membros = rows
                    .filter(row => row.id_documento === r.id_documento)
                    .map(row => ({
                        id_conta: row.membro_id,
                        nome_utilizador: row.membro_nome,
                        email: row.membro_email,
                        linguas_membro: row.linguas_membro,
                        assinou_documento: row.assinou_documento === 1
                    }));
                ...
                documentosMap.set(r.id_documento, {
                    ...
                    erros_encontrados: r.erros_encontrados,
                    valor: r.valor,
                    paginas: r.paginas,
...
router.post('/documentos/:idDocumento/assinatura', verifyToken, verifyRole([3, 4]), async (req, res) => {
    const { idDocumento  } = req.params;
    const { conta_id, assinou_documento } = req.body;

    if (!idDocumento || !conta_id || typeof assinou_documento === 'undefined') {
        return res.status(400).json({ message: 'Parâmetros inválidos' });
    }
     
    try {
        const [result] = await db.execute(
            `INSERT INTO equipa_assinaturas (conta_id, documento_id, assinou_documento)
            VALUES (?, ?, ?)
            ON DUPLICATE KEY UPDATE assinou_documento = VALUES(assinou_documento)`,
            [conta_id, idDocumento, assinou_documento]
        );

        if (req.user.cargo_id == 4) {
            const [docInfo] = await db.execute(
                `SELECT ed.responsavel_upload_id
                FROM equipa_documentos ed
                JOIN equipas e ON ed.equipa_id = e.id_equipa
                WHERE ed.documento_id = ?
                AND e.tipo = 'revisores'`,
                [idDocumento]
            );

            const responsavelId = docInfo[0]?.responsavel_upload_id;

            // Verifica se todos os revisores assinaram
            const [assinaturas] = await db.execute(
                `SELECT ea.conta_id, ea.assinou_documento
                FROM equipa_assinaturas ea
                JOIN equipa_documentos ed ON ed.documento_id = ea.documento_id
                JOIN equipas e ON ed.equipa_id = e.id_equipa
                WHERE ea.documento_id = ? 
                AND e.tipo = 'revisores'
                AND ea.conta_id != ?`, // EXCLUI RESPONSÁVEL
                [idDocumento, responsavelId]
            );

            const todosAssinaram = assinaturas.every(a => a.assinou_documento == 1);

            if (todosAssinaram) {
                // Só atualiza para finalizado se não houver erros e não estiver em_traducao
                await db.query(
                    `UPDATE documentos
                    SET estado = CASE 
                        WHEN (erros_encontrados = 'Sem Erros')
                            AND estado != 'em_traducao'
                        THEN 'finalizado'
                        ELSE estado
                    END,
                    documento_link_final = CASE
                        WHEN (erros_encontrados IS NULL OR erros_encontrados = '' OR erros_encontrados = 'sem_erros')
                            AND estado != 'em_traducao'
                        THEN link_traduzido
                        ELSE link_final
                    END
            ...
            console.log(`Documento ${idDocumento}: todosAssinaram=${todosAssinaram}`);
        }

        res.json({ success: true, todosAssinaram: todosAssinaram || false });
    } catch (err) {
        console.error(err);
...
router.put('/documentos/:idDocumento/revisao', verifyToken, verifyRole([4]), async (req, res) => {
    const { idDocumento } = req.params;
    const { erros_encontrados, estado } = req.body; 
     ...
     try {
        ...
        if (rows[0].responsavel_upload_id !== req.user.id) {
            console.log('responsavel_upload_id (DB):', rows[0].responsavel_upload_id);
            console.log('req.user:', req.user);

            return res.status(403).json({ message: 'Sem permissão para rever este documento' });
        }

        await db.query( `
            UPDATE documentos
            SET erros_encontrados = ?, estado = IFNULL(?, estado)
            WHERE id_documento = ?
        `, [erros_encontrados, estado, idDocumento]);
