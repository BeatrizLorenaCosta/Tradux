...
router.get('/equipas', verifyToken, async (req, res) => {
    try {
        const [rows] = await db.query(`
            SELECT
                e.id_equipa AS id,
                e.nome_equipa AS nome,
                e.tipo,

                COUNT(DISTINCT em.conta_id) AS membros,

                GROUP_CONCAT(
                    DISTINCT l.nome_lingua
            ...
            LEFT JOIN linguas l ON l.id_lingua IN (d.lingua_origem, d.lingua_destino)
