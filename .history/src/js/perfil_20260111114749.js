...
const updateNavMenu = () => {
    ...
    // Limpa itens dinâmicos antigos
    nav.querySelectorAll('.dynamic-link').forEach(item => item.remove());
    ...
    // Helper para criar itens do menu lateral
    const criarItem = (sectionId, texto, icon) => {
        const div = document.createElement('div');
        div.className = 'nav-item dynamic-link';
        ...
        div.addEventListener('click', () => {
            ...
            mostrarSecao(sectionId); // função que já deves ter
        });
        ...
        if (user) {
            ...
            if (user.cargo_id === 1) {
                // Admin deve mostrar: Alterar dados(sessao), sair
            } else if (user.cargo_id === 2) {
                // Cliente deve mostrar: Meus Documentos(sessao), Alterar dados(sessao), sair
            } else if (user.cargo_id === 3) { 
                // Tradutor deve mostrar: Alterar dados(sessao), Tradução (que tem sub items com: Equipa de tradução do utilizador (sessao), Documentos da equipa(sessao)), sair
            } else if (user.cargo_id === 4) { 
                // Revisor deve mostrar: Alterar dados(sessao), Revisao (que tem sub items com: Equipa de revisado do utilizador (sessao), Documentos da equipa(sessao)), sair
            }
