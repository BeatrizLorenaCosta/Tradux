...
abrirFormAlterarCargo = async (userId) => {
    ...
    // tem certeza que quer alterar o cargo
    form.onsubmit = async (e) => {
        e.preventDefault();
        const novoCargoId = form.querySelector('select').value;

        try {
            const res = await fetch(`/api/admin/users/${user.id_conta}/cargo`, {
                method: 'PUT',
                headers: getAuthHeaders(),
                body: JSON.stringify({ cargo_id: novoCargoId })
            });
