    ...
    try {
        ...
        renderTabela(tbodyDocs,
            equipa.documentos.map(d => {
                ...
                const totalMembros = d.equipa.membros.length;
                const assinaturas = d.equipa.membros.filter(m => m.assinou_documento).length;

                // Verifica se todos menos o responsável assinaram
                const podeUploadNoForm = assinaturas >= (totalMembros - 1);
                ...
                return `
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn btn-sm btn-info ms-1" onclick="abrirFormAssinatura(${d.id_documento})">
                                Assinar
                            </button>
                        </td>
                    </tr>
...
// Abre o modal/form de assinatura de um documento
async function abrirFormAssinatura(id_documento) {
    try {
        // Busca o documento e equipa via API ou do objeto já carregado
        const documento = equipa.documentos.find(d => d.id_documento === id_documento);
        if (!documento) return alert('Documento não encontrado');

        // Limpar modal
        const modalBody = document.getElementById('modalAssinaturaBody');
        modalBody.innerHTML = '';

        // Lista os membros da equipa
        const ul = document.createElement('ul');
        documento.equipa.membros.forEach(m => {
            const li = document.createElement('li');

            // Se for o utilizador atual, checkbox para assinar
            if (m.id_conta === user.id) {
                const checkbox = document.createElement('input');
                checkbox.type = 'checkbox';
                checkbox.checked = m.assinou_documento;
                checkbox.onchange = async () => {
                    await assinarDocumento(documento.id_documento, m.id_conta, checkbox.checked);
                    // Atualiza o estado local
                    m.assinou_documento = checkbox.checked;
                    abrirFormAssinatura(id_documento); // re-render
                };
                li.appendChild(checkbox);
                li.append(` ${m.nome_utilizador}`);
            } else {
        ...
        // Verifica se todos os membros (menos o responsável) assinaram
        const totalMembros = documento.equipa.membros.length;
        const assinaturas = documento.equipa.membros.filter(m => m.assinou_documento).length;

        const podeUpload = (assinaturas >= totalMembros - 1) && (user.id === documento.responsavel_upload_atual_id);

        if (podeUpload) {
            const btnUpload = document.createElement('button');
            btnUpload.className = 'btn btn-warning mt-2';
            btnUpload.textContent = 'Fazer Upload do ficheiro final';
            btnUpload.onclick = () => uploadFicheiro(documento.id_documento);
            modalBody.appendChild(btnUpload);
        }

        // Mostrar modal (usando Bootstrap 5 como exemplo)
        const modal = new bootstrap.Modal(document.getElementById('modalAssinatura'));
        modal.show();

    } catch (err) {
        console.error('Erro ao abrir form de assinatura:', err);
    }
}

// Função para assinar o documento via API
async function assinarDocumento(id_documento, membro_id, assinar) {
    try {
        await apiFetch(`/api/documentos/${id_documento}/assinar`, {
            method: 'POST',
            body: { membro_id, assinar }
        });
    } catch (err) {
...
function uploadFicheiro(id_documento) {
    // Aqui abrir um input file ou redirecionar para form de upload
    const inputFile = document.createElement('input');
    inputFile.type = 'file';
    inputFile.accept = '.pdf,.docx'; // ajusta conforme necessário
    inputFile.onchange = async () => {
        ...
        try {
            await apiFetch(`/api/documentos/${id_documento}/upload`, {
                method: 'POST',
                body: formData
            });
            alert('Upload concluído!');
            location.reload(); // ou atualizar tabela dinamicamente
        } catch (err) {
