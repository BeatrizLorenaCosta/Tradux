    ...
    try {
        ...
        renderTabela(tbodyDocs,
            equipa.documentos.map(d => {
                ...
                // Verifica se o utilizador logado é o responsável pelo upload
                const podeUpload = d.responsavel_upload_atual_id === user.id;

                // Verifica se todos membros assinaram
                const todosAssinaram = d.equipa.membros.every(m => m.assinou_documento);

                // Verifica se o utilizador já assinou
                const jaAssinou = d.equipa.membros.find(m => m.id_conta === user.id)?.assinou_documento;
...
${podeUpload ? `<button class="btn btn-sm btn-warning ms-1" onclick="abrirUpload(${d.id_documento})">Fazer Upload</button>` : ''}

                    ${!jaAssinou && todosAssinaram ? `<button class="btn btn-sm btn-info ms-1" onclick="assinarDocumento(${d.id_documento})">Assinar</button>` : ''}
                </td>
                    </tr>
                `;
            }),
