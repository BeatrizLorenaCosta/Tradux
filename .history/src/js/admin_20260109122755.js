...
if (formDocEquipaTradutores) {
    ...
    formDocEquipaTradutores.addEventListener('submit', async e => {
        ...
        try {
            ...
            carregarEquipasSelect();
            carregarUtilizadoresPorTipo();
            alert('Documento associado a equipa de tradutores com sucesso');
        } catch (err) {
            console.error(err);
            alert(err.message);
        }
    });
} else if (formDocEquipaRevisores) {
    const selects = formDocEquformDocEquipaRevisoresipaTradutores.querySelectorAll('select');

    const selDoc = selects[0];
    const selTrad = selects[1];
    const selRev = selects[2];

    carregarDocumentosSelect(selDoc, 'tevisores');
    carregarEquipasLivres(selTrad, 'revisores');
    ...
    formDocEquipaRevisores.addEventListener('submit', async e => {
        ...
        try {
            ...
            carregarEquipasSelect();
            carregarUtilizadoresPorTipo();
            alert('Documento associado a equipa de revisores com sucesso');
        } catch (err) {
            console.error(err);
            alert(err.message);
        }
