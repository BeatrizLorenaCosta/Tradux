                ...
                return `
                    <tr>
                        ...
                        <td>
                            ...
                            ` : ''}
                            ${(d.estado === 'em_revisao' || d.estado === 'aguardando_assinaturas') ? `
                                <button class="btn btn-sm btn-info ms-1"
                                    onclick="abrirFormAssinatura(${d.id_documento}, '${tipo}')">
                                    Assinar
                                </button>
                            ` : ''}
...
async function abrirFormAssinatura(id, tipo) {
    ...
    // Configuração de erros (só revisores)
    if (tipo === 'revisores') {
        const inputErros = form.querySelector('[name="erros_encontrados"]');
        const checkboxSemErrosWrapper = form.querySelector('#sem-erros').closest('.form-check');
        const checkboxSemErros = form.querySelector('#sem-erros');
        const btnValidar = document.getElementById('btn-validar-revisor');

        const temErros = documento.erros_encontrados && documento.erros_encontrados.trim() !== '' && documento.erros_encontrados !== 'Sem Erros';
        inputErros.value = temErros ? documento.erros_encontrados : '';
        checkboxSemErros.checked = !temErros;
        inputErros.disabled = checkboxSemErros.checked;

        if (isResponsavel) {
            msgRevisao.textContent = "É necessário guardar a revisão antes de assinar!";
            msgRevisao.style.display = 'inline-block';
            checkboxSemErrosWrapper.style.display = 'block';
            checkboxSemErros.disabled = false;
            btnValidar.style.display = 'inline-block';

            checkboxSemErros.onchange = () => {
                inputErros.disabled = checkboxSemErros.checked;
                if (checkboxSemErros.checked) inputErros.value = '';
            };
            ...
            btnValidar.onclick = async () => {
                ...
                const checkbox = ul.querySelector(`#membro-${user.id_conta}`);
                if (checkbox) checkbox.disabled = false;
                msgRevisao.style.display = 'none';
            ...
            checkboxSemErrosWrapper.style.display = 'none';
            btnValidar.style.display = 'none';
        }
    }

    // Lista membros para assinatura
    if (documento.equipa.membros.length === 0) {
            ...
            if (m.id_conta === user.id_conta) {
                li.innerHTML = `
                    <div class="form-check">
                        <input class="form-check-input me-2" type="checkbox" id="membro-${m.id_conta}" 
                            ${m.assinou_documento ? 'checked' : ''} 
                            ${tipo === 'revisores' && !revisaoGuardada && isResponsavel ? 'disabled' : ''}>
                        <label class="form-check-label mt-1" for="membro-${m.id_conta}">${m.nome_utilizador}</label>
                    </div>
                `;
                const checkbox = li.querySelector('input[type="checkbox"]');
                checkbox.onchange = async () => {
                    if (!revisaoGuardada && tipo === 'revisores' && isResponsavel) {
                        msgRevisao.style.display = 'block';
                        checkbox.checked = false;
                        return;
                    }
                    msgRevisao.style.display = 'none';
                    await assinarDocumento(documento.id_documento, m.id_conta, checkbox.checked, tipo);
    ...
    if (btnFechar) btnFechar.onclick = () => { wrapper.style.display = 'none'; form.reset(); };
...
function atualizarUploadBotao(documento, user) {
    ...
    if (tipoEquipa === 'tradutores') {
        ...
        if (documento.documento_link_traduzido) {
            btn.style.display = 'inline-block';
            btn.onclick = async () => {
                await assinarDocumento(documento.id_documento, user.id_conta, true, tipo);
...
async function assinarDocumento(idDocumento, idConta, assinou, tipo) {
    ...
    const btnFechar = wrapper.querySelector('.btn-fechar-form');
    if (btnFechar) btnFechar.click(); // simula clique no botão de fechar
}
...
async function guardarRevisaoDocumento(idDocumento, errosEncontrados) {
    ...
    if (errosEncontrados && errosEncontrados.trim() !== '') 
        payload.estado = 'em_traducao';
