...
async function carregarDocumentos() {
    ...
    const rows = docs.map(d => {
        ...
        const btnAlterarValor = d.estado === 'em_analise'
            ? `<button class="btn btn-sm btn-primary btn-alterar-valor" onclick="abrirFormAlterar${d.id_documento}, this, 'valorDoc')"> Alterar Valor</button>`
            : '';
...
async function carregarUtilizadores() {
    ...
    const rows = users.map(u => `
        <tr>
            ...
            <td>${u.nome_cargo}</td>
            <td>
                <button class="btn btn-sm btn-primary" onclick="abrirFormAlterarCargo(${u.id_conta}, this, 'cargoUt')">
                    Alterar Cargo
                </button>
