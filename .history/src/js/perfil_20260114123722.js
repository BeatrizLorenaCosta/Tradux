    ...
    try {
        ...
        renderTabela(tbodyDocs,
            equipa.documentos.map(d => {
                ...
                // Verifica se todos os membros da equipa já assinaram
                const todosAssinaramMenosResponsavel = d.equipa.membros
                    .filter(m => m.id_conta !== d.responsavel_upload_atual_id)
                    .every(m => m.assinou_documento);

                // Verifica se o utilizador já assinou
                const jaAssinou = d.equipa.membros.find(m => m.id_conta === user.id)?.assinou_documento;

                // Verifica se o utilizador é responsável pelo upload
                const souResponsavel = user.id === d.responsavel_upload_atual_id;
