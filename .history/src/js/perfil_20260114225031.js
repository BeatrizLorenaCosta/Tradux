...
async function abrirFormAssinatura(id, btn, tipo) {
    const equipa = await apiFetch('/api/users/me/equipa');
    const user = await apiFetch('/api/users/me');
    const jaAberto = btn.dataset.open === '1';
    ...
    if (tipo === 'revisores') {
        ...
        if (isResponsavel) {
            console.log(documento.erros_encontrados);
            if ((documento.erros_encontrados && 
    documento.erros_encontrados.trim() !== '') || documento.erros_encontrados === 'Sem Erros') {
                console.log("chegouusdasd");
            ...
            if (m.id_conta === user.id_conta) {
                ...
                checkbox.onchange = async () => {
                    ...
                    if (!revisaoGuardada && tipo == 'revisores') {
                        ...
                        if (timeoutMsgRevisao) {
                            clearTimeout(timeoutMsgRevisao);
                        }

                        timeoutMsgRevisao = setTimeout(() => {
                            msgRevisao.style.display = 'none';
                        }, 3000);
                        return;
                    }
    ...
    if (tipoEquipa === 'tradutores') {
        ...
        if (documento.documento_link_traduzido) {
            ...
            btn.onclick = async () => {
                ...
                abrirFormAssinatura(documento, btn, tipoEquipa);
            };
        } else {
            btn.style.display = 'none';
        }
