...
async function abrirFormAssinatura(id, btn, tipo) {
    ...
    const errosInput = form.querySelector('[name="erros_encontrados"]');
    const isResponsavel = user.id_conta === documento.responsavel_upload_atual?.id_conta;

    // Se for responsável, habilitar input e checkbox
    errosInput.disabled = !isResponsavel;
    if (isResponsavel) {
        // Se não houver erros ainda, mostrar placeholder ou checkbox
        errosInput.value = documento.erros_encontrados || '';
    }
