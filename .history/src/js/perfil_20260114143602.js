...
async function abrirFormAssinatura(id, btn, tipo) {
    ...
    let btnFechar = document.getElementById("btn-fechar-form");
    btnFechar.onclick = () => {
...
async function assinarDocumento(idDocumento, idConta, assinar) {
    console.log(assinar);
    await apiFetch(`/api/documentos/${idDocumento}/assinatura`, {
        method: PUT,
        body: JSON.stringify({ conta_id: idConta, assinou_documento: assinar ? 1 : 0 })
    });
}
