...
function abrirFormAssinatura(documento, tipoEquipa) {
    // Escolher wrapper e form conforme o tipo de equipa
    const wrapper = tipoEquipa === 'tradutores' 
        ? document.getElementById('form-tradutor-wrapper') 
        : document.getElementById('form-revisor-wrapper');

    const form = tipoEquipa === 'tradutores' 
        ? document.getElementById('form-tradutor') 
        : document.getElementById('form-revisor');

    // Preencher hidden e nome do documento
    form.id_documento.value = documento.id_documento;
    form.nome_documento.value = documento.nome_documento;

    // Limpar lista anterior
    const ul = tipoEquipa === 'tradutores' 
        ? document.getElementById('assinaturas-tradutores') 
        : document.getElementById('assinaturas-revisores');
    ul.innerHTML = '';

    // Lista membros com checkbox para o utilizador atual
    documento.equipa.membros.forEach(m => {
        const li = document.createElement('li');
        li.className = 'list-group-item d-flex justify-content-between align-items-center';

        // Checkbox apenas se for membro atual
        if (m.id_conta === user.id) {
            ...
            checkbox.onchange = async () => {
                ...
                atualizarUploadBotao(documento, tipoEquipa);
            };
            li.appendChild(checkbox);
            li.appendChild(document.createTextNode(` ${m.nome_utilizador}`));
        } else {
    ...
    // Mostrar ou ocultar botão de upload ou validar
    atualizarUploadBotao(documento, tipoEquipa);
...
function atualizarUploadBotao(documento, tipoEquipa) {
    ...
    if (tipoEquipa === 'tradutores') {
        ...
        if ((assinaturas >= totalMembros - 1) && (user.id === documento.responsavel_upload_atual_id)) {
        ...
        if (documento.documento_link_traduzido) {
            btn.style.display = 'inline-block';
            btn.onclick = async () => {
                await assinarDocumento(documento.id_documento, user.id, true);
                document.equipa.membros.find(m => m.id_conta === user.id).assinou_documento = true;
                abrirFormAssinatura(documento, tipoEquipa); // atualizar lista
            };
        } else {
            btn.style.display = 'none';
        }
