...
async function abrirFormAssinatura(id, tipo) {
    ...
    let isResponsavel = user.id_conta === documento.responsavel_upload_atual?.id_conta;
    ...
    if (tipo === 'revisores') {
        ...
        if (documento.erros_encontrados && documento.erros_encontrados.trim() !== '' && documento.erros_encontrados !== 'Sem Erros') {
            inputErros.value = documento.erros_encontrados;
            checkboxSemErros.checked = false;
            inputErros.disabled = false;
        } else {
            inputErros.value = '';
            checkboxSemErros.checked = true;
    ...
    const membrosParaAssinatura = documento.equipa.membros;
    ...
    if (membrosParaAssinatura.length === 0) {
        ...
        if (!isResponsavel && (documento.erros_encontrados === null || documento.erros_encontrados === '')) {
            msgRevisao.textContent = "O responsavel deve adicionar erros encontrados antes de assinar!";
            msgRevisao.style.display = 'inline-block';
        }
                ...
                li.innerHTML = `
                    <div class="form-check">
                        <input class="form-check-input me-2" 
                            type="checkbox" 
                            id="membro-${m.id_conta}" 
                            ${m.assinou_documento ? 'checked' : ''} 
                            ${(!revisaoGuardada && tipo === 'revisores' && isResponsavel) || (documento.erros_encontrados === null || documento.erros_encontrados === '') ? 'disabled' : ''}>
                        <label class="form-check-label mt-1" for="membro-${m.id_conta}">
                            ${m.nome_utilizador}
                        </label>
                    </div>
                `;
...
async function guardarRevisaoDocumento(idDocumento, errosEncontrados) {
    ...
    await apiFetch(`/api/users/documentos/${idDocumento}/revisao`, {
        method: 'PUT',
        body: JSON.stringify({ erros_encontrados: errosEncontrados})
    });
}
