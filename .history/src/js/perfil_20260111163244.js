...
async function carregarEquipa() {
    try {
        ...
        formMembros.tipo_equipa.value = equipa.tipo;
        formMembros.estado_equipa.value = equipa.tipo === 'tradutores' ? 'Ocupada' : 'Livre';

        const tbodyMembros = document.querySelector('#equipa-tradutores table tbody');
        tbodyMembros.innerHTML = equipa.membros.length
            ? equipa.membros.map(e => `
                <tr>
                    <td>#${String(e.id_conta).padStart(3,'0')}</td>
                    <td>${e.nome_utilizador}</td>
                    <td>${e.email}</td>
                    <td>${e.linguas_membro}</td>
                </tr>
            `).join('')
            : `<tr><td colspan="4">Nenhum membro encontrado.</td></tr>`;
        ...
        document.getElementById('tipo_equipa2').value = equipa.tipo;
        document.getElementById('estado_equipa2').value = equipa.tipo === 'tradutores' ? 'Ocupada' : 'Livre';

        const tbodyDocs = document.querySelector('#documentos-equipa table tbody');
        const [l, c] = mapEstado(equipa.documentos.estado);
        console.log(equipa.documentos.estado);

        tbodyDocs.innerHTML = equipa.documentos && equipa.documentos.length
            ? equipa.documentos.map(d => `
                <tr>
                    <td>#TRX-${String(d.id_documento).padStart(3,'0')}</td>
